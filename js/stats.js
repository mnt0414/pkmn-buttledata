...
function renderStats(){
  ...
  const wr_color=pct>=50?'var(--win)':'var(--loss)';
  document.getElementById('stat-sum').innerHTML=`
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:9px;margin-bottom:4px;">
      <div class="card" style="text-align:center;padding:13px;">
        ...
        <div style="font-size:10px;color:var(--text3);margin-top:4px;letter-spacing:.06em;">対戦数</div>
      </div>
      <div class="card" style="text-align:center;padding:13px;">
        <div class="num" style="font-size:26px;font-weight:800;line-height:1;color:var(--win);">${wins}</div>
        <div style="font-size:10px;color:var(--text3);margin-top:4px;letter-spacing:.06em;">勝利数</div>
        <div class="num" style="font-size:10.5px;color:var(--text2);margin-top:2px;">${wins}/${total}</div>
      </div>
      <div class="card" style="text-align:center;padding:13px;">
        <div class="num" style="font-size:26px;font-weight:800;line-height:1;color:${wr_color};">${pct}%</div>
        <div style="font-size:10px;color:var(--text3);margin-top:4px;letter-spacing:.06em;">勝率</div>
      </div>
    </div>`;
    ...
    if(!isPair){
      ...
      row.innerHTML=`<span style="width:30px;height:30px;border-radius:8px;flex:none;background:var(--raised);display:flex;align-items:center;justify-content:center;overflow:hidden;">${spriteImg(name,30)}</span>
        ...
        <div style="flex:1;height:6px;background:var(--line);border-radius:6px;overflow:hidden;">
          <div class="bar-fill" style="height:100%;width:${pct}%;background:var(--lead);border-radius:6px;"></div>
        </div>
        <span class="num" style="font-size:12px;color:var(--text2);width:36px;text-align:right;">${pct}%</span>`;
    }else{
...
function cwRate(id,bs,side,top,limit){
  ...
  sorted.forEach(item=>{
    ...
    const col=pct>=60?'var(--win)':pct<=40?'var(--loss)':'var(--text2)';
...
function renderCross(bs){
  ...
  let html=`<table style="border-collapse:collapse;font-size:10px;"><thead><tr><th style="padding:5px 8px;color:var(--text3);border-bottom:1px solid var(--line);">自分↓ / 相手→</th>`;
  oppTop.forEach(ok=>html+=`<th style="padding:5px 8px;color:var(--text3);border-bottom:1px solid var(--line);white-space:nowrap;">${ok.replace('＋','<br>')}</th>`);
  html+='</tr></thead><tbody>';
  myTop.forEach(mk=>{
    html+=`<tr><th style="padding:5px 8px;color:var(--text3);white-space:nowrap;text-align:left;">${mk.replace('＋','<br>')}</th>`;
    oppTop.forEach(ok=>{
      const m=bs.filter(b=>b.myLead.slice().sort().join('＋')===mk&&b.oppLead.slice().sort().join('＋')===ok);
      if(!m.length){html+='<td style="padding:5px 8px;color:var(--text3);text-align:center;">-</td>';return}
      ...
      html+=`<td style="padding:5px 8px;background:${bg};color:${tc};font-weight:600;text-align:center;">${pct}%<br><span style="font-size:8px;color:var(--text3)">${w}/${m.length}</span></td>`;
    });html+='</tr>';
  });
...
function renderHist(bs){
  ...
  bs.slice(0,40).forEach(b=>{
    ...
    row.innerHTML=`<span class="num" style="font-size:10px;color:var(--text3);width:42px;flex:none;">${ds}</span>
